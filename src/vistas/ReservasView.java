...
public class ReservasView extends JFrame{
    ...
    public ReservasView() {
		...
		txtFechaE.setFont(new Font("Roboto", Font.PLAIN, 12));
		panel.add(txtFechaE);
		...
		lblValorSimbolo.setFont(new Font("Roboto", Font.BOLD, 15));
		...
		txtFechaS.setFont(new Font("Roboto", Font.PLAIN, 18));
                //enviando fecha inicial al ejecutar
                String fecha1 = "2022/09/01";
                SimpleDateFormat formato = new SimpleDateFormat("yyyy/MM/dd");
                try {
                    Date fechaDate = formato.parse(fecha1);
                    txtFechaE.setDate(fechaDate);
                } catch (ParseException ex) {
                    Logger.getLogger(ReservasView.class.getName()).log(Level.SEVERE, null, ex);
                }
                //enviando fecha final al ejecutar
                String fecha2 = "2022/09/30";
                try {
                    Date fechaDate2 = formato.parse(fecha2);
                    txtFechaS.setDate(fechaDate2);
                } catch (ParseException ex) {
                    Logger.getLogger(ReservasView.class.getName()).log(Level.SEVERE, null, ex);
                }
		...
		txtValor.setBounds(100, 328, 103, 33);
		txtValor.setEditable(false);
		txtValor.setFont(new Font("Roboto Black", Font.BOLD, 18));
		txtValor.setBorder(javax.swing.BorderFactory.createEmptyBorder());
		panel.add(txtValor);
		txtValor.setColumns(10);                                
                //**************Evento dentro del JdateChooser para calcular diferencia entre fechas********** 
		txtFechaS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
                            Date fechaInicial = txtFechaE.getDate();                            
                            Date fechaFinal = txtFechaS.getDate();                        
                            nuevReserva.setFechaInicial(new java.sql.Date(fechaInicial.getTime()));
                            nuevReserva.setFechaFinal(new java.sql.Date(fechaFinal.getTime()));
                            ...
                            int valor=nuevReserva.calcularValorReserva(fechaIniLocal, fechaFinLocal);
                            nuevReserva.setValor(valor);                            
                            txtValor.setText("$ "+String.valueOf(nuevReserva.getValor()));
			}
		});
		...
		txtFormaPago.setFont(new Font("Roboto", Font.PLAIN, 13));
